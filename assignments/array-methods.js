...
// Problem 1
// Creates an array of contacts including first name and last initialfollowed by email:
let contactList = runners.map((runner) => {
    return `Name: ${runner.first_name} ${runner.last_name[0]} Email: ${runner.email}`
});
console.log(contactList); 

// Problem 2
// Create an array of all needed shirt sizes
let shirts = runners.map((runner) => {
    return runner.shirt_size;
});
console.log(shirts);

// Problem 3
// Filter shirts that are larger into an additional array
let bigShirts = runners.filter((runner) => {
    if (runner.shirt_size === "XL" || runner.shirt_size === "2XL" || runner.shirt_size === "3XL"){
        return runner.shirt_size;
    }
});
